...
| 모델명 | 특징 및 용도 |
|--------|-------------|
| **Gemini 1.5 Pro** | 가장 강력하고 다양한 작업에 적합. 매우 긴 컨텍스트(최대 100만 토큰) 지원. 복잡한 문서 분석, 코드 생성, 멀티모달 작업 등 |
| **Gemini 1.5 Flash** | Pro와 유사한 성능에 더 빠르고 저렴함. 대규모 실시간 작업, 요약, 데이터 추출 등 |
| **Gemini 1.0 Pro** | 이전 세대 모델. 일반 텍스트 생성, 간단한 질의응답 등 |

> **모델 선택 팁:** 복잡한 작업에는 `Gemini 1.5 Pro`, 빠른 응답과 비용 효율이 중요하면 `Gemini 1.5 Flash`를 추천합니다.
